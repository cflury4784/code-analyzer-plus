...
> **C4 compliance guard**: All PRs in Phase 1b must not introduce new inline JSON parsing (brace-tracking, manual `JSON.parse` with swallowed errors). They must also not worsen existing C4 violations in files they touch. C4 (`extractJson` extraction) is a Current Invariant — enforced in all PRs today. If a Phase 1b change touches `analyze.ts`, `dedup.ts`, or `src/phases/index.ts`, the engineer must verify no new inline parsing is added even if the full Phase 2.2 extraction is deferred.
...
- **Action**:
  1. **Create `src/llm-client.ts`** containing the `LLMClient` interface with a single request method and shared abort signal lifecycle.
  2. Extract common error handling, abort patterns, and timeout logic into the interface implementation base.
  3. Replace all hardcoded endpoint strings with environment config or a config object.
  4. Refactor `src/lm-studio.ts` and `src/lms-rest.ts` to implement `LLMClient` (imported from `src/llm-client.ts`).
...
- **Action**:
  1. **Create `src/phase-orchestrator.ts`** exporting class `PhaseOrchestrator`, responsible for: main execution loop, retry/backoff, manifest state reads and writes.
...
- **Findings**:
  ...
  - Tight coupling via shared `src/types.ts` dependency.
- **Action**:
  1. **Create `src/fs-service.ts`** containing the `FileSystemService` interface defining all I/O operations used by phase modules.
  2. Inject `FileSystemService` as a **function parameter** on each exported function in phase modules — phase modules stay as function exports, not classes.
  3. Refactor `aggregate.ts` and `dedup.ts` to accept `FileSystemService` as a parameter and call it instead of importing `fs` directly.
...
- **Action**:
  1. **Create the new file** `tests/utils/TestEnvironmentManager.ts`. `TestEnvironmentManager` must return fresh, independent state on every call — never a shared mutable instance.
  2. Migrate logic from `tests/utils/envHelpers.ts` into `TestEnvironmentManager`. Once all callers are migrated, **delete** `tests/utils/envHelpers.ts`. `tests/utils/fsHelpers.ts` is migrated similarly; delete it when empty.
  3. Refactor test files with duplicated `beforeEach`/`afterEach` env-setup blocks to use `TestEnvironmentManager` instead.
...
- **Done when**: No `process.platform === 'win32'` conditionals exist outside the adapter; a unit test asserts which exclusion rule wins when both a dynamic exclusion and a `.gitignore` entry apply to the same path (dynamic wins).
...
- **Action**:
  1. **Create the new file** `tests/utils/fixtures.ts` (it does not exist yet) and implement `createTestManifestFixture` and `generatePromptFixture` in it. Use these wherever the same shape appears in 2+ test files. One-off test objects with ≤5 properties used only in a single `describe` block may remain inline.
