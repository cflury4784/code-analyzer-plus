...
export async function estimateTotalGB(
  ...
  const raw = await deps.runLms([
    'load', loadKey, '--parallel', '1', '--context-length', String(contextLength), '--estimate-only',
  ]);
  // Example line: "Estimated Total Memory: 18.52 GiB"
  const m = raw.match(/Estimated Total Memory:\s*([\d.]+)\s*GiB/i);
  if (!m) {
...
- `--estimate-only` parsing keys off the literal `Estimated Total Memory:` line; keep the regex
  tolerant of extra spacing and ANSI color (callers should pass plain text — `lms` `--json` is
  not available for `load`, so this parses human output; strip is not required because the regex
  is anchored on the label, but if ANSI codes wrap the number, widen the regex to
  `/Estimated Total Memory:[^\d]*([\d.]+)\s*GiB/i`).
- `LoadedModel` is intentionally open (`[k: string]: unknown`) — we only depend on `identifier`.

### 5. Validation & Testing
- `npx tsc --noEmit` passes.
- These wrappers are unit-tested in `tests/preflight.test.ts` (Phase 3) via a mocked `runLms`,
  including the defensive-parse scenarios (malformed/zero-exit non-JSON → labeled error).
