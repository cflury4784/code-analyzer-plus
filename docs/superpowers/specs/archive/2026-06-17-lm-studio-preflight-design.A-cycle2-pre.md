...
- **Malformed `lms` output.** Each wrapper that parses `--json` must not call `JSON.parse`
  bare. Wrap parsing and validate the expected shape (e.g. `serverStatus` → object with
  boolean `running`; `listLoaded` → array of objects with string `identifier`). On parse
  failure or shape mismatch — even when `lms` exits zero — throw a labeled `Error`
  (`"lms <cmd> returned unparseable output: <stdout snippet>"`). This protects against
  `lms` version drift, prepended warning lines, or empty stdout, keeping the
  "distinct labeled error" guarantee.
...
7. **Malformed `lms` output** — `serverStatus`/`listLoaded` return non-JSON or wrong shape on
   a zero exit → throws a labeled error (not a bare `SyntaxError`).
