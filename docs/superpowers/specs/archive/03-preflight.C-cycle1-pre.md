...
```ts
import { describe, it, expect, vi } from 'vitest';
import { serverStatus, listLoaded, load, estimateTotalGB, type runLms as RunLmsT } from '../src/lms.js';

type RunLms = typeof RunLmsT;
...
- `npx tsc --noEmit` clean (note the `type runLms as RunLmsT` import is a type-only import of
  the function's type via `typeof`; if the implementer prefers, export an explicit `RunLms`
  type from `lms.ts` and import that instead).
