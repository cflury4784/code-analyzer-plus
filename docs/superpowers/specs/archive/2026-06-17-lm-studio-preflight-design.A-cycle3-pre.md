...
estimate cannot be parsed, `estimateTotalGB` throws and preflight falls back to a
conservative `requiredTotalGB = os.totalmem()` GiB (require the whole box) so the abort path
stays safe rather than optimistic.
...
4. **Resource check.** (`os.totalmem()` is true capacity here — no carveout, see Host context.)
   - `requiredTotalGB` = registry value; or, for an unknown override,
     ...
     `estimateTotalGB` throws (unparseable), fall back to `requiredTotalGB = os.totalmem()` GiB.
