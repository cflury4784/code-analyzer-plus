...
**Provides**: `src/fs-service.ts` (FileSystemService interface + NodeFileSystemService); `aggregate.ts` and `dedup.ts` refactored; T1 standard activated
...
**Out of scope**: `src/phases/index.ts` and `src/phases/analyze.ts` — also have `fs` imports but deferred. Do not touch in this PR.
...
After both phase files are modified, update all call sites to pass `new NodeFileSystemService()`.

```powershell
npx grep -rn "runAggregatePhase|runDedupPhase" src/
```
...
// In runPipeline, pass fsService as 4th argument:
await runAggregatePhase(this, projectRoot, resolvedModel, this.logger, fsService, ...);
await runDedupPhase(this, projectRoot, resolvedModel, this.logger, fsService, ...);
```
...
- [ ] T1 standard now enforced (noted in PR description)
