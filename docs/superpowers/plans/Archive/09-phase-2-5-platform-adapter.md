...
## File 4: `tests/unit/discovery.test.ts` (APPEND — do not create new file)

Phase 2.3 already migrated this file to use `TestEnvironmentManager`. Append this `describe` block to the existing file.

**Note on TS1 compliance**: Phase 2.3 prohibits inline `mkdtempSync`/`rmSync` in `beforeEach`/`afterEach`. Use `setupTempFs` from `TestEnvironmentManager` instead.
